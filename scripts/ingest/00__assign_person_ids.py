...
Skips **low-quality** faces: normalized bbox area (``width * height``) below ``MIN_BBOX_AREA``
...
def incremental_assign(conn: sqlite3.Connection, face_ids: list[str]) -> None:
    if not face_ids:
        raise RuntimeError(
            "No candidate faces (check PREFIXES, MIN_BBOX_AREA, and that images are indexed)."
        )
...
def main() -> None:
    conn = init_db()
    try:
        face_ids = load_target_face_ids(conn, min_bbox_area=MIN_BBOX_AREA, prefixes=PREFIXES)
        print(f"Prefixes: {PREFIXES!r}")
        print(f"MIN_BBOX_AREA: {MIN_BBOX_AREA}")
