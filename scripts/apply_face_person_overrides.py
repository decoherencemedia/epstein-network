...
Run from scripts/:
  python3 apply_face_person_overrides.py
  python3 apply_face_person_overrides.py path/to/overrides.json
...
OVERRIDE_JSON = Path(__file__).resolve().parent / "face_person_overrides.json"
...
def main() -> None:
    ...
    json_path = Path(sys.argv[1]) if len(sys.argv) > 1 else OVERRIDE_JSON
    if not json_path.is_absolute():
        json_path = (scripts_dir / json_path).resolve()
