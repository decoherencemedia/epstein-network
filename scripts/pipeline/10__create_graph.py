...
# Manual node face crops: run ``13__optimize_node_faces.py`` to build WebPs from
...
_MODERATION_HINT = (
    "Some images with faces have no or invalid moderation_result; you may need to run "
    "scripts/pipeline/06__moderate_images.py (after 04__index_faces so rows are indexed)."
)


def _assert_moderation_results_for_faces_df(df: pd.DataFrame) -> None:
    """
    Every image that contributes faces to this graph must have a stored DetectModerationLabels
    JSON object. Missing SQLite NULL / pandas NaN, empty strings, non-JSON, or JSON that
    parses to a non-object (e.g. a bare number) all fail fast with a clear pipeline hint.
    """
    by_img = df.drop_duplicates(subset=["image_name"], keep="first")
    missing: list[str] = []
    bad: list[tuple[str, str]] = []
    for _, row in by_img.iterrows():
        name = str(row["image_name"])
        raw = row["moderation_result"]
        ...
        if not isinstance(raw, str):
            bad.append((name, f"expected str, got {type(raw).__name__}"))
            continue
        try:
        ...
        if isinstance(data, float):
            bad.append((name, "JSON parsed to float (expected a moderation object)"))
            continue
        if not isinstance(data, dict):
            bad.append((name, f"JSON top-level type is {type(data).__name__}, expected object"))
            continue
    if not missing and not bad:
        return
    parts = [_MODERATION_HINT, ""]
    if missing:
        parts.append(
            f"Missing or empty moderation_result ({len(missing)} image(s)), e.g.: "
            f"{', '.join(missing[:15])}" + (" ..." if len(missing) > 15 else "")
        )
    if bad:
        parts.append("Invalid moderation_result:")
        for n, err in bad[:20]:
            parts.append(f"  {n}: {err}")
        if len(bad) > 20:
            parts.append(f"  ... and {len(bad) - 20} more")
    raise RuntimeError("\n".join(parts))
...
def _is_explicit_moderation(moderation_result: str | None) -> bool:
    ...
    if not isinstance(data, dict):
        raise RuntimeError(
            f"moderation_result must be a JSON object (DetectModerationLabels response), "
            f"got {type(data).__name__}. {_MODERATION_HINT}"
        )
...
if __name__ == "__main__":
    ...
    _assert_moderation_results_for_faces_df(df)
